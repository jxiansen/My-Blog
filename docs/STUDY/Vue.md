  ...
  export default {
    data() {
      return {
        ...
        imgSrc: 'http://i0.hdslb.com/bfs/album/e6c53dc180e8e1a5dcd819cc161ee0fdc911128d.png'
      }
    },
  }
</script>
...
### `V-show`
v-show通过设置标签的display属性来实现标签的隐藏和显示
...
``` html {1,8-11}
<h1 v-for="(item,index) in arrayObj">{{ index+1 }}.职位:{{ item.work }} 工作地:{{item.place}} 薪水:{{item.salary}} </h1>
